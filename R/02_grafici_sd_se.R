...
# ============================================================
# PLOT 5 — Three-species comparison with SD (use for descriptive)
# ============================================================
# calcola media e deviazione standard per specie
df_sp <- do.call(data.frame,
                 aggregate(Petal.Length ~ Species, data = iris,
                           FUN = function(x) c(mean = mean(x), sd = sd(x))))
names(df_sp) <- c("Species", "mean", "sd")

# grafico con ± SD
p5 <- ggplot(df_sp, aes(x = Species, y = mean, fill = Species)) +
  geom_col(alpha = 0.80, width = 0.55) +
  ...
  labs(
    title    = "Mean \u00b1 SD of petal length across three Iris species",
    subtitle = "Using standard deviation instead of standard error",
    x = NULL, y = "Petal length (cm)",
    caption  = "Error bars = \u00b1 1 SD  |  Data: iris (Fisher 1936)"
  ) +
  base_theme +
  theme(legend.position = "none")
...
ggsave("img/plot5_species_comparison.png", p5, width=8, height=5, dpi=150, bg="white")
...
message("  - img/plot5_species_comparison.png")
