...
# Colour palette (consistent with the series theme)
col_sd  <- "#C0623C"   # rust  — SD
col_se  <- "#2d6a4f"   # green — SE
col_med <- "#1A1A2E"   # dark  — mean
col_ci  <- "#1565c0"   # blue  — 95% CI
...
# ============================================================
# PLOT 1 — Individual value distribution + SD
# ============================================================
p1 <- ggplot(df, aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)),
                 binwidth = 0.1, fill = "#a8d8b0",
...
# ============================================================
# PLOT 3 — Sampling distribution of the mean (bootstrap)
# ============================================================
set.seed(42)
...
p3 <- ggplot(data.frame(m = boot_means), aes(x = m)) +
  ...
  labs(
    ...
    subtitle = "The SE equals the SD of this distribution: it quantifies uncertainty of the mean estimate",
    x        = "Sample mean (cm)",
    y        = "Density",
...
# ============================================================
# PLOT 4 — Three-species comparison with SE error bars (inferential use)
# ============================================================
...
p4 <- ggplot(df_sp, aes(x = Species, y = mean, fill = Species)) +
  ...
  labs(
    ...
    subtitle = "Correct inferential use of SE in comparative bar plots",
    x        = NULL,
    y        = "Petal length (cm)",
